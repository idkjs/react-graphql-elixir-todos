...
@bs.val @bs.module("jsonwebtoken")
external verifyExternal: (string, string, verifyOptions) => Js.t<'a> = "verify"

@bs.deriving(jsConverter)
type token = {clientId: string}

exception InvalidJWT(exn)

let verify = (token, cert, options) =>
  switch verifyExternal(token, cert, options) {
  | value => Belt.Result.Ok(tokenFromJs(value))
  | exception e => Error(InvalidJWT(e))
  }

type decodeOptions = {"complete": bool}
type decodeExp = {"exp": int}
...
external decode: (
  string,
  decodeOptions,
) => Js.Nullable.t<decodeResult> = "decode"
@bs.val @bs.module("jsonwebtoken")
external decodeExp: (
  string,
  decodeExp,
...
let getKeyId: string => option<string> = token =>
  decode(token, {"complete": true})
  ->Js.Nullable.toOption
  ->Belt.Option.flatMap(result =>
    Js.Nullable.toOption(result->headerGet)->Belt.Option.flatMap(header =>
      Js.Nullable.toOption(header->kidGet)
    )
  )

export decodeJwt: string => option<string> = token =>
  decode(token, {"complete": true})
  ->Js.Nullable.toOption
  ->Belt.Option.flatMap(result =>
    Js.Nullable.toOption(result->headerGet)->Belt.Option.flatMap(header =>
